...
function getCompletion(series: LibrarySeries) {
  const total = series.total_number_of_episodes ?? 0;

  if (total > 0) {
    const watched = series.total_number_of_episodes_watched ?? 0;
    return Math.min(100, Math.round((watched / total) * 100));
  }

  return series.watch_status === WATCH_STATUS[2].value ? 100 : 0;
}

export function SeriesCard({ series }: { series: LibrarySeries }) {
  const completion = getCompletion(series);
  const isLongTitle = series.name.length > 18;
  const episodeInfo = series.total_number_of_episodes
    ? `EP ${series.total_number_of_episodes_watched ?? 0} OF ${series.total_number_of_episodes}`
    : "Series";
